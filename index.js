  ...
  async (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (email) {
        ...
        // If this is a Founding Member (lifetime) purchase, cancel any active
        // trial/subscription on the same customer so they don't get charged later.
        if (session.metadata?.plan === 'lifetime' && session.customer) {
          try {
            ...
            for (const sub of activeSubs.data) {
              ...
              console.log(`[webhook] auto-cancelled subscription ${sub.id} for founding member ${email}`);
            }
            ...
            for (const sub of trialSubs.data) {
              await stripe.subscriptions.cancel(sub.id);
              console.log(`[webhook] auto-cancelled trial subscription ${sub.id} for founding member ${email}`);
            }
          } catch (cancelErr) {
